...
function printBoard(boardData){
	...
	for(let i = 0 ; i < boardData.length ; i++){
		console.log( String.fromCharCode(65+i)+" | "+boardData[i].join(" | ")+" |");
		console.log( "  +"+"---+".repeat(boardData.length));
	}
}
...
function placeInThatPosition(boardData,fleetMark, fleetLength, placementRow, placementColumn, placementOrientation) {
...
function checkPlacement(boardData, placementRow, placementColumn, placementOrientation, fleetLength){
...
function rollRandomPlacement(boardData,fleetLength){
	...
	//generate sideways / downward (1  = sideway, - downward)
	...
	if(placementOrientation){
		...
		placementColumn = generateRandomIntFromZeroTo(boardData.length - fleetLength );
...
function generateEnemyOnBoard(boardData,fleetData){
	for(let fleetUnit in fleetData){
		...
		while(!placementIsValid){
			let randomPLacementResult = rollRandomPlacement(boardData,fleetData[fleetUnit].size);
			let checkPlacementResult = checkPlacement(boardData,randomPLacementResult[1],randomPLacementResult[2],randomPLacementResult[0], fleetData[fleetUnit].size);
			if(checkPlacementResult){
				placeInThatPosition(boardData,fleetData[fleetUnit].mark,fleetData[fleetUnit].size,randomPLacementResult[1],randomPLacementResult[2],randomPLacementResult[0]); break;
			}
		}
	}
}
...
function checkIfFleetSink(fleetData,fleetMark){
	let shipLife = fleetData[getFleetNameFromFleetMark(fleetData ,fleetMark)].life;
...
function interactWithTheTarget(boardData, fleetData,targetRow, targetColumn){
	if(boardData[targetRow][targetColumn] === "X") {
		...
		if(checkIfFleetSink(fleetData,fleetThatisHit)){
			...
			return "You Hit "+ getFleetNameFromFleetMark(fleetData,fleetThatisHit) +" fleet!";
		}
...
function startBattleShip(playerGuesses){
	...
	if(playerGuesses.length >=10){
		...
		for(let onePLayerGuess of playerGuesses){
			...
			let message = interactWithTheTarget(boardData, fleetData, targetRow, targetColumn);
			printBoard(boardData);
